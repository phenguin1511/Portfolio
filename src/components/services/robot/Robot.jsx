...
export function Robot(props) {
  ...
  // Log các animation có sẵn để kiểm tra (tùy chọn)
  useEffect(() => {
    console.log('Available animations:', names);
  }, [names]);

  // Tự động chạy animation khi component mount
  useEffect(() => {
    if (names.length > 0) {
      const action = actions[names[0]]; // Chạy animation đầu tiên (có thể thay bằng tên cụ thể)
      action
        .reset() // Đặt lại về frame đầu tiên
        .fadeIn(0.5) // Hiệu ứng mờ vào trong 0.5 giây
        .play(); // Chạy animation

      // Lặp lại animation (tùy chọn)
      action.setLoop(THREE.LoopRepeat, Infinity); // THREE được import từ three-stdlib

      // Dừng animation khi component unmount (tùy chọn)
      return () => {
        action.fadeOut(0.5); // Hiệu ứng mờ ra khi dừng
      };
    }
  }, [actions, names]);
  return (
